...
angular.module("myApp.NFRTable", ['ngRoute'])
  ...
  .controller("NFRTableController", ["$scope", "$document", "service", "$http", function ($scope, $document, service, $http) {
    ...
    self.filterQueryComplexity=function(nfrDefalutValue){
      try{
        var tmpJson = JSON.parse(JSON.stringify(nfrDefalutValue));
        delete tmpJson['queryComplexity'];
      }
      catch{
      }
      return tmpJson;
    ...
    self.updateNFRValues = function () {
      if (self.checkNFRValues($scope.nfrDefalutValue) == true) {
        self.UpdateTheJSON();
        $('#nfrValuesModal').modal('hide');
      }
      else {
        $scope.errorMessage = "The Sum Of The NFR Values Must Be Equal To 1";
      }
    }

    self.cancelUpdateNFRValues = function () {
      $('#nfrValuesModal').modal('hide');
      $scope.nfrDefalutValue = JSON.parse($scope.prevNFRValues);
    }

    self.openUpdatingModal = function () {
      $scope.errorMessage = "";
      $scope.prevNFRValues = JSON.stringify($scope.nfrDefalutValue);
      $('#nfrValuesModal').modal('show');
    }

    self.checkNFRValues = function (nfrValues) {
      var sum = 0;
      var number = true;
      for (var key in nfrValues) {
        sum = sum + parseFloat(nfrValues[key]["value"]);
        number = /^-?\d*\.?\d*$/.test(nfrValues[key]["value"]);
        if (number == false) {
          return false;
        }
      }
    ...
    self.castToNumber = function (string) {
      return parseFloat(string);
    }

    $scope.updatedNfrValues = {};
    self.UpdateTheJSON()
