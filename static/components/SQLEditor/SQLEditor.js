angular.module('myApp.SQLEditor', ['ngRoute'])
  ...
  .controller("SQLEditorController", ["$scope", "$document", "service", "$http", "$window", function ($scope, $document, service, $http, $window) {
    ...
    self.addQuery = function () {
      //new Query structure
      var newQuery = { "name": "query", "body": "", "selected": true, "queryType": "", "joinClass": [], "queryComplexity": "", "frequency": 10 ,"message":""};
    ...
    self.setInputAttributes = function (object) {
      ...
      $scope.queryName = object.name;
      $scope.frequency = object.frequency;
      $scope.queryComplexity = object.queryComplexity;
      setQueryType(object.queryType);
      arrSelectedClassForJoin = object.joinClass;
      if ($scope.SQLTextArea.length > 0) {
        parseSQL();
    ...
    self.saveQuery = function () {
      ...
      $scope.SQLQueriesList[indexOfOpenQuery].queryComplexity = $scope.queryComplexity;
      $scope.SQLQueriesList[indexOfOpenQuery].message = $scope.message1;
    ...
    self.setWhiteSpaceIfItsUndifinded = function () {
      ...
      if ($scope.queryComplexity == undefined) {
        $scope.queryComplexity = "";
      }
    ...
    function setFirstQuery() {
      ...
      if ($scope.SQLQueriesList == 0) {
        self.addQuery();
        self.openQuery($scope.SQLQueriesList[0]);
      }
    };
    ...
    $('#joinClasses2').dblclick(function () {
      try {
        ...
        if (index !== -1) {
          var tmp = (arrSelectedClassForJoin.slice(0, index)).concat(arrSelectedClassForJoin.slice(index + 1, arrSelectedClassForJoin.length));
          if (checkRemainClass(tmp)) {
    ...
    function checkRemainClass(arrOfClasses) {
      if (arrOfClasses.length > 1) {
        for (var i = 1; i < arrOfClasses.length; i++) {
          var newArr = (arrOfClasses.slice(0, i - 1)).concat(arrOfClasses.slice(i, arrOfClasses.length));
          if (!checkLinkBetween(newArr, arrOfClasses[i - 1])) {
            return false;
          }
        }
      }
    ...
    function checkLinkBetween(currentArr, newValue) {
      $scope.message1="";
      $scope.SQLQueriesList[indexOfOpenQuery]["message"]="";
      if (currentArr.length == 1) {
        if (newValue == currentArr[0]) {
          return true;
        }
      }
      ...
      if (currentArr.length > 0) {
        ...
        if (flag == false) {
          $scope.message1="There Is No Link Between '" + newValue + "' Class And Your Others Class";
          $scope.SQLQueriesList[indexOfOpenQuery]["message"]="There Is No Link Between '" + newValue + "' Class And Your Others Class";
          return false;
        }
        return flag;
      }
    ...
    function parseSQL() {
      ...
      checkLinkBetween([],"");
      if (checkRemainClass(arrSelectedClassForJoin)) {
        $scope.queryComplexity = getQueryComplexity(query, arrSelectedClassForJoin);
        updateDynamicSelectjoinClass(arrSelectedClassForJoin);
      }
    }
    ...
    // function checkDirectLink(listOfClasses) {
    //   sendRequestUML();
    //   //if we have only one class
    //   if (listOfClasses.length > 0 && listOfClasses != undefined) {
    //     var i;
    //     var legend = umlData[0];
    //     // the uml matrix
    //     var data = [];
    ...
    //     }
    //     var flag = true;
    //     // check if each class have direct link with other classes
    //     for (i = 0; i < listOfClasses.length; i++) {
    //       var oldValue = listOfClasses[i];
    //       for (j = 0; j < listOfClasses.length && j != i; j++) {
    //         var newValue = listOfClasses[j];
    //         var indexOfOldValue = legend.indexOf(oldValue);
    //         var indexOfNewValue = legend.indexOf(newValue);
    //         if (data[indexOfOldValue][indexOfNewValue] == 0) {
    //           flag = false;
    //         }
    //       }
    //     }
    ...
    function getCyclicJoinValue(listOfClasses) {
      if (listOfClasses.length < 2) {
        return 1
      }
      else {
        for (var i = 0; i < listOfClasses.length; i++) {
          for (var j = 0; j < listOfClasses.length; j++) {
            if ((listOfClasses[i] == listOfClasses[j]) && (i != j)) {
              return 5;
            }
          }
        }
    ...
    function getQueryComplexity(query, listOfClasses) {
      var queryComplexity = 0;
      var returnFlag = checkIfContain(query, "return");
      var fromFlag = checkIfContain(query, "from");
      var whereFlag = checkIfContain(query, "where");
      var sqlFunctionsFlag = false;
      var cyclicJoinValue = getCyclicJoinValue(listOfClasses);
      for (var i = 0; i < self.sqlFunctions.length; i++) {
        if (checkIfContain(query, self.sqlFunctions[i])) {
          sqlFunctionsFlag = true;
          break;
        }
      }
      if (returnFlag && fromFlag) {
        queryComplexity = 1;
      }
      if (whereFlag) {
        queryComplexity = 2;
      ...
      return Math.max(queryComplexity, cyclicJoinValue);
    }
    ...
    function textAreaSetSetting() {
      ...
      categoryTags = ["and", "as", "between", "delete", "from", "insert", "or", "return", "set", "where", "update", "connect", "min", "max", "count", "avg", "sum"];
    ...
    self.queryTypeList = ["select", "insert", "update", "delete", "connect"];
    // values of the query complexity
    self.queryComplexityList = [1, 2, 3, 4, 5];
    self.sqlFunctions = ["min", "max", "count", "avg", "sum"];
