angular.module("myApp.Login", ['ngRoute'])
  ...
  .controller("LoginController", ["$scope", "service", '$http', '$window', '$location', '$rootScope', '$document', function ($scope, service, $http, $window, $location, $rootScope, $document) {
    ...
    self.Login = function () {
      $http.post('/login', $scope.User).then(function (response) {
        try {
          if (response.data.Email == "admin") {
          ...
          else if (response.data.data != "Login failed ,User Name Or Password Is Incorrect" && response.data!="" ) {
            ...
            $window.alert("Login failed ,User Name Or Password Is Incorrect");
          }
        }
        catch (err) {
    ...
    var project = { "projectName": "mytest", "Introdunction": "project Name : test 1 \n\n\nproject description : my description" };

    //function to rest the password if the user forget it
