...
# this function return dictinary of {nfr:nfr weights} for project from the project json
def getNFRDefaultValueDict(prjectJson):
    tmp ={"ProjectId":prjectJson[4]}
    project=mongo_func.getProject(tmp)
    nfrDict={}
    for nfr , nfrJson in project["ProjectJson"]["NFRDefalutValue"].items():
        nfrDict[nfr]=nfrJson['value']
    return nfrDict