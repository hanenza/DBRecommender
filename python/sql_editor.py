...
def getClassesComplexity(request):
    classValues={}
    classCount={}
    finalDict={}
    for i in range(1, len(request)):
        if (request[i]["joinClass"] != None and request[i]["selected"]==True):
            for j in range(0,len(request[i]["joinClass"])):
                try:
                    classValues[request[i]["joinClass"][j]]=classValues[request[i]["joinClass"][j]]+request[i]["queryComplexity"]
                    classCount[request[i]["joinClass"][j]]=request[i]["joinClass"]+1
                except:
                    classValues[request[i]["joinClass"][j]]=request[i]["queryComplexity"]
                    classCount[request[i]["joinClass"][j]]=1
    for key, value in classValues.items():
        finalDict[key]=classValues[key]/classCount[key]
    return finalDict
